...
class ExpensePage(CTkFrame):
    ...
    def add_fields(self):
        ...
        self.amount = (CTkEntry(master=grid, fg_color="#F0F0F0", border_width=0, width=300))
        ...
        self.category = CTkComboBox(master=grid, width=300,
                                    ...
                                    button_color="#2A8C55", border_color="#2A8C55", button_hover_color="#207244",
                                    dropdown_hover_color="#207244", dropdown_fg_color="#2A8C55",
                                    dropdown_text_color="#fff", font=("Arial", 15))
        print('category ',self.category.get())
        self.category.grid(
            row=1,
            column=1,
            ipady=10,
        ...
        CTkLabel(master=grid, text="Description", font=("Arial Bold", 17), text_color="#52A476", justify="left").grid(
            ...
            column=0,
            sticky="w")
        self.description = CTkEntry(master=grid, fg_color="#F0F0F0", border_width=0, width=300, height=140)
        print('description ',self.description.get())
        ...
        calendar = CustomCalendar(grid, selectmode='day', year=2022, month=5, day=22)
        calendar.configure(background='#52A476', foreground='white')
        calendar.grid(row=15, column=1, padx=(24, 0), pady=15)

        # add payment method
        CTkLabel(master=grid, text="Payment method", font=("Arial Bold", 17), text_color="#52A476",
                 justify="left").grid(
            row=27,
            column=0,
            sticky="w")
        ...
        CTkRadioButton(master=grid, variable=self.payment_method, value=0, text="Cash", font=("Arial Bold", 14),
                       text_color="#52A476", fg_color="#52A476", border_color="#52A476", hover_color="#207244").grid(
            row=30, column=0, sticky="w", pady=(16, 0))
        CTkRadioButton(master=grid, variable=self.payment_method, value=1, text="Card", font=("Arial Bold", 14),
                       text_color="#52A476", fg_color="#52A476", border_color="#52A476", hover_color="#207244").grid(
            row=31, column=0, sticky="w", pady=(16, 0))
        CTkRadioButton(master=grid, variable=self.payment_method, value=2, text="Online", font=("Arial Bold", 14),
                       text_color="#52A476", fg_color="#52A476", border_color="#52A476", hover_color="#207244").grid(
            row=32, column=0, sticky="w", pady=(16, 0))
        ...
        # add button
        CTkButton(master=self, text="ADD", width=150, height=150, fg_color="#2A8C55", hover_color="#207244",
                  font=("Arial Black", 20), command=self.add_expense_to_user).pack(anchor="e", ipady=5, pady=(0, 25), padx=27)

    def add_expense_to_user(self):
        curr_expense = Expense(self.amount.get(), self.category.get(), self.description.get(), self.payment_method.get())
        print(curr_expense)
